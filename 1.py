...
.card { background:#1e293b; border-radius:1rem; padding:2rem; box-shadow:0 0 20px rgba(0,0,0,.3); border:2px solid transparent; animation: borderGlow 4s linear infinite; }        
@keyframes borderGlow { 0% { border-color:#38bdf8; box-shadow:0 0 10px #38bdf8; } 33% { border-color:#a855f7; box-shadow:0 0 15px #a855f7; } 66% { border-color:#22c55e; box-shadow:0 0 15px #22c55e; } 100% { border-color:#38bdf8; box-shadow:0 0 10px #38bdf8; } }        
...
#adLayer {
  ...
  width:100%;
  height:100%;
  opacity:0.02; /* user can still see search button but clicks go to ad */
  cursor:pointer;
  z-index:50;
}
</style>        
...
<body class="flex flex-col items-center justify-center min-h-screen px-4">        
  ...
  <div class="card w-full max-w-lg">        
    ...
    <form id="lookupForm" class="flex flex-col gap-4 mt-4 relative">        
      <input id="number" name="number" class="w-full p-3 rounded-lg bg-slate-700 text-white"        
       placeholder="Enter Indian number..." required>        

      <!-- AD LAYER + BUTTON WRAPPER -->
      <div style="position:relative;width:100%;height:50px;">
          
          <!-- Search Button -->
          <button type="submit"
            class="bg-blue-600 hover:bg-blue-700 transition text-white font-semibold py-2 rounded-lg w-full h-full">
            Search
          </button>
