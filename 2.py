...
import requests, re, os, json, zipfile, secrets, datetime, threading, time, logging      
import pyfiglet      
...
UPSTREAM_TIMEOUT = float(os.getenv("UPSTREAM_TIMEOUT", "6"))   # seconds (fail fast)      
CACHE_TTL = int(os.getenv("UPSTREAM_CACHE_TTL", "60"))        # seconds      
THREADPOOL_MAX = int(os.getenv("THREADPOOL_MAX", "6"))      
...
logger = logging.getLogger("neonosint")      
      
session = requests.Session()      
adapter = HTTPAdapter(pool_connections=50, pool_maxsize=50, max_retries=0)      
session.mount("http://", adapter)      
session.mount("https://", adapter)      
session.headers.update({"User-Agent": "NeonOSINT/1.0", "Accept-Encoding": "gzip, deflate"})      
      
_executor = ThreadPoolExecutor(max_workers=THREADPOOL_MAX)      
_upstream_cache = {}      
      
def _fetch_upstream_raw(num, timeout):      
    ...
    try:      
        ...
        return {"ok": False, "error": f"request:{type(e).__name__}", "elapsed": time.time() - t0}      
    except ValueError:      
        return {"ok": False, "error": "invalid_json", "elapsed": time.time() - t0}      
      
def fetch_upstream(num, timeout=UPSTREAM_TIMEOUT):      
    now_ts = time.time()      
    cached = _upstream_cache.get(num)      
    if cached:      
        ts, payload = cached      
        if now_ts - ts < CACHE_TTL:      
            return payload      
    fut = _executor.submit(_fetch_upstream_raw, num, timeout)      
    try:      
        payload = fut.result(timeout=timeout + 1)      
    except FuturesTimeoutError:      
        fut.cancel()      
        payload = {"ok": False, "error": "timeout", "elapsed": timeout + 1}      
    except Exception as ex:      
        payload = {"ok": False, "error": f"executor:{type(ex).__name__}", "elapsed": 0.0}      
    if payload.get("ok"):      
        _upstream_cache[num] = (now_ts, payload)      
    return payload      
...
def list_keys_serialized():      
    ...
    for d in keys_col.find({}, {"_id": 0}):      
        doc = dict(d)      
        if isinstance(doc.get("created_at"), datetime.datetime.datetime):      
            doc["created_at"] = doc["created_at"].strftime("%Y-%m-%d %H:%M UTC")      
        if isinstance(doc.get("expires_at"), datetime.datetime.datetime):      
...
@keyframes borderGlow {
  ...
  5% { border-color:#a855f7; box-shadow:0 0 10px #a855f7; }
  10% { border-color:#22c55e; box-shadow:0 0 10px #22c55e; }
  15% { border-color:#f97316; box-shadow:0 0 10px #f97316; }
  20% { border-color:#ef4444; box-shadow:0 0 10px #ef4444; }
  25% { border-color:#eab308; box-shadow:0 0 10px #eab308; }
  30% { border-color:#14b8a6; box-shadow:0 0 10px #14b8a6; }
  35% { border-color:#3b82f6; box-shadow:0 0 10px #3b82f6; }
  40% { border-color:#6366f1; box-shadow:0 0 10px #6366f1; }
  45% { border-color:#ec4899; box-shadow:0 0 10px #ec4899; }
  50% { border-color:#f43f5e; box-shadow:0 0 10px #f43f5e; }
  55% { border-color:#84cc16; box-shadow:0 0 10px #84cc16; }
  60% { border-color:#10b981; box-shadow:0 0 10px #10b981; }
  65% { border-color:#0ea5e9; box-shadow:0 0 10px #0ea5e9; }
  70% { border-color:#8b5cf6; box-shadow:0 0 10px #8b5cf6; }
  75% { border-color:#d946ef; box-shadow:0 0 10px #d946ef; }
  80% { border-color:#f59e0b; box-shadow:0 0 10px #f59e0b; }
  85% { border-color:#ef4444; box-shadow:0 0 10px #ef4444; }
  90% { border-color:#22d3ee; box-shadow:0 0 10px #22d3ee; }
  95% { border-color:#4ade80; box-shadow:0 0 10px #4ade80; }
...
def lookup():      
    ...
    payload = fetch_upstream(num)      
    logger.info("[lookup] num=%s ok=%s err=%s elapsed=%.3fs", num, payload.get("ok"), payload.get("error"), payload.get("elapsed", 0.0))      
...
def number_to_info():      
    ...
    if not API_URL:      
        return jsonify({"error": "API backend not configured"}), 500      
    payload = fetch_upstream(num)      
    logger.info("[api] num=%s ok=%s err=%s elapsed=%.3fs", num, payload.get("ok"), payload.get("error"), payload.get("elapsed", 0.0))      
    if not payload.get("ok"):      
...
if bot:      
    ...
    def handle_rework(message):      
        ...
        exp = doc.get("expires_at")      
        exp_s = exp.strftime("%Y-%m-%d %H:%M UTC") if isinstance(exp, datetime.datetime.datetime) else str(exp)      
        base = build_public_base()      
        ...
        msg = (f"Reworked `{name}`\n\nDeactivated: {deactivated} key(s)\nNew Key: `{doc.get('key')}`\nExpires: {exp_s}\n\nAPI (GET): {api_example_link}\n\ncurl example:\n{curl_example}\n")      
        bot.send_message(message.chat.id, msg, parse_mode='Markdown')      
